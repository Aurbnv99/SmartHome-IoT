...
import os
import csv
from collections import deque   # efficient fixed-size ring buffer
...
### --- NEW: CSV LOGGING INITIALIZATION ---
...
def ai_energy_thread():
    ...
    # ------------------------------------------------------------------ #
    #  TIMING CONSTANTS                                                    #
    #  SEQUENCE_LENGTH : number of 1-second samples the LSTM needs        #
    #  PRED_INTERVAL   : how often (s) we run the LSTM inference          #
    #  CSV_LOG_INTERVAL: how often (s) we append a row to the CSV         #
    #  PRINT_INTERVAL  : how often (s) we print an AI UPDATE line         #
    # ------------------------------------------------------------------ #
    SEQUENCE_LENGTH   = 60   # unchanged – model input shape stays the same
    PRED_INTERVAL     = 5    # predict every 5 s  (was: 60 s → caused flat line)
    CSV_LOG_INTERVAL  = 60   # keep CSV at 1 row / minute
    PRINT_INTERVAL    = 10   # console spam guard
    ...
    # ------------------------------------------------------------------ #
    #  BIAS CORRECTION                                                     #
    #  The model has a ~0.5 kW systematic offset (seen in the 2-h run).   #
    #  We track the rolling mean of (actual - raw_prediction) over the    #
    ...
    while True:
        try:
            ...
            # ---------------------------------------------------------- #
            #  BUILD PROXY FEATURES  (identical logic – nothing changed)  #
            # ---------------------------------------------------------- #
            ...
            # ---------------------------------------------------------- #
            #  FRIDGE – exponential smoothing (Fix 1)                     #
            #  Old code did a hard 50x jump every 30 min which sent the   #
            #  LSTM input distribution off a cliff. Smoothing keeps the   #
            #  transition gradual so the model stays in distribution.     #
            # ---------------------------------------------------------- #
            ...
            # ---------------------------------------------------------- #
            #  KEY FIX 1: ADD TO BUFFER EVERY SECOND                      #
            #  Previously gated behind a 60-s timer, so 60 entries meant  #
            #  60 MINUTES of history – the model was always 1 hour stale. #
            #  Now 60 entries = 60 SECONDS, so the model reacts quickly.  #
            # ---------------------------------------------------------- #
            df_row = pd.DataFrame([raw])[FEATURES]
            buffer.append(df_row.values[0])   # deque auto-drops oldest

            # ---------------------------------------------------------- #
            #  KEY FIX 2: PREDICT EVERY 5 s (not every 60 s)             #
            # ---------------------------------------------------------- #
            if len(buffer) == SEQUENCE_LENGTH:
                ...
                if current_time_sec - last_pred_time >= PRED_INTERVAL:
                    ...
                    # -------------------------------------------------- #
                    #  KEY FIX 3: ROLLING BIAS CORRECTION                 #
                    #  Accumulate (actual - raw_pred) errors.             #
                    #  Once we have ≥3 samples the correction activates.  #
                    #  This removes the ~0.5 kW systematic offset seen    #
                    #  in the 2-hour run (R²=−1790, flat prediction line).#
                    # -------------------------------------------------- #
                ...
                # CSV log (once per minute – same rate as before)
                ...
                # KEY FIX 4: during the 60-second warm-up, mirror curr_kw
                # so the dashboard never shows a dead-flat 0 kW line.
                pred_real = curr_kw
            ...
            # ---------------------------------------------------------- #
            #  SMART METER LOGIC  (guard: only active when model is live) #
            # ---------------------------------------------------------- #
            if model_ready:
            ...
            # ---------------------------------------------------------- #
            #  SAVE TO JSON DASHBOARD (every second, for real-time chart) #
            #  Added "model_ready" flag so the dashboard can show a       #
            #  "Buffering…" banner during the first 60 s.                 #
            # ---------------------------------------------------------- #
            # sample = {
            ...
            # # Scrittura su file temporaneo e poi rinomina atomica
